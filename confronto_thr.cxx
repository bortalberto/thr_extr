...
#include "stdio.h"
#include <string>
void export_thr_L1(int run_num1, int run_num2)

{
    TString iname = "thr_run_";
    iname += Form("%i", run_num1);
    iname += ".root";
    TString iname2 = "thr_run_";
    iname2 += Form("%i", run_num2);
    iname2 += ".root";
    auto datafile = new TFile(iname);
    auto datatree = (TTree*)datafile->Get("tree");
    auto datafile2 = new TFile(iname2);
    auto datatree2 = (TTree*)datafile2->Get("tree");
    ...
    datatree2->Draw("thr_t_fC:strip_x>>h32", "layer_id==1 && strip_x>0 ","same");
    ...
    datatree2->Draw("thr_t_fC:strip_v>>h42", "layer_id==1 && strip_v>0 ","same");
    ...
    datatree2->Draw("thr_t_fC>>h52", "strip_v>0 && layer_id==1");
    h5->Draw();
    h52->Draw("same");
    int max;
    ...
    datatree2->Draw("thr_t_fC>>h62", "strip_x>0 && layer_id==1");
    h6->Draw();
    h62->Draw("same");
    ...
    legend9->AddEntry(h6,Form("T branch %d, mu= %.2f, rms=%.2f",run_num1,m1,rms1),"f");
    legend9->AddEntry(h62,Form("T branch %d, mu= %.2f, rms=%.2f",run_num2,m2,rms2),"f");
    legend9->Draw();
...
void export_thr_L2(int run_num1,int run_num2)

{
    ...
    datatree2->Draw("thr_t_fC:strip_x>>h32", "layer_id==2 && strip_x>0 ","same");
    ...
    datatree2->Draw("thr_t_fC:strip_v>>h42", "layer_id==2 && strip_v>0 ","same");
    ...
    datatree2->Draw("thr_t_fC>>h52", "strip_v>0 && layer_id==2");
    h5->Draw();
    h52->Draw("same");
    int max;
    if (h5->GetMaximum() > h52->GetMaximum()){
    ...
    datatree2->Draw("thr_t_fC>>h62", "strip_x>0 && layer_id==2");
    h6->Draw();
    h62->Draw("same");
